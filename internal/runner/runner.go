...
type RunConfig struct {
	...
	Format     OutputFormat  // text, json, junit (default: text)
...
func Run(cfg RunConfig) (RunResult, error) {
	...
	if cfg.Format == FormatJSON || cfg.Format == FormatJUnit {
		report = &jsonReport{}
	}
			...
			if report != nil {
				...
				for _, r := range results {
					jr.Assertions = append(jr.Assertions, jsonAssertion{
						Expression: r.Assertion.Raw,
						Passed:     r.Passed,
						Actual:     r.Actual,
						Error:      r.Error,
					})
				}
				report.Requests = append(report.Requests, jr)
			}
	...
	if report != nil {
		...
		if cfg.Format == FormatJSON {
			writeJSON(cfg.Output, report)
		} else if cfg.Format == FormatJUnit {
			writeJUnit(cfg.Output, report, cfg.FilePath)
		}
	}
